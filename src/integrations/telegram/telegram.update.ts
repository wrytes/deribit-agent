...
export class TelegramUpdate implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      await this.bot.telegram.setMyCommands([
        ...
        { command: 'status',     description: 'Account info and Deribit connection' },
        { command: 'market',     description: 'Current prices, DVOL, IV rank' },
  ...
  async onStart(@Ctx() ctx: Context) {
    ...
    const { isNew } = await this.authService.getOrCreateUser(BigInt(from.id), from.username);
    const greeting = isNew ? 'Welcome to Deribit Agent! 🤖\n\n' : 'Welcome back! 👋\n\n';
  ...
  async onStatus(@Ctx() ctx: Context) {
    ...
    lines.push(`Telegram: @${from.username ?? from.first_name}\nID: \`${from.id}\``);
  ...
  async onPositions(@Ctx() ctx: Context) {
    ...
    try {
      ...
      if (active.length === 0) { await ctx.reply('📭 No open positions.'); return; }
  ...
  async onOrders(@Ctx() ctx: Context) {
    ...
    try {
      ...
      if (allOrders.length === 0) { await ctx.reply('📭 No open orders.'); return; }
  ...
  private async handleConnectWizard(
    ...
    if (step === 'client_secret') {
      ...
      await ctx.reply('✅ Connected! Credentials saved.\n\nTry /balance or /positions.');
