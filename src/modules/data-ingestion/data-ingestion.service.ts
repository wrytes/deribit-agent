...
export class DataIngestionService implements OnModuleInit {
  ...
  private async runInitialBackfillIfNeeded() {
    for (const { instrument, resolution } of TRACKED_INSTRUMENTS) {
      const count = await this.prisma.candle.count({ where: { instrument, resolution } });
      if (count === 0) {
        const from = new Date(Date.now() - INITIAL_LOOKBACK_DAYS * 86_400_000);
        this.logger.log(`No candles for ${instrument}@${resolution} — backfilling ${INITIAL_LOOKBACK_DAYS}d`);
        const result = await this.backfillCandles({ instrument, resolution, from });
        this.logger.log(`Initial backfill done: ${instrument}@${resolution} → ${result.inserted} candles`);
      }
    }
  }
  ...
  async backfillCandles(dto: BackfillDto): Promise<BackfillResult> {
    ...
    while (cursor < toTs) {
      ...
      const data = await this.fetchChartData(dto.instrument, dto.resolution, cursor, chunkEnd);
      ...
      for (let i = 0; i < ticks.length; i++) {
        ...
        try {
          await this.prisma.candle.upsert({
            where: {
              instrument_resolution_timestamp: { instrument: dto.instrument, resolution: dto.resolution, timestamp },
            },
            update: {},
            create: {
  ...
  async ingestAllTracked(): Promise<{ instrument: string; resolution: string; inserted: number }[]> {
    const results: { instrument: string; resolution: string; inserted: number }[] = [];
    for (const { instrument, resolution } of TRACKED_INSTRUMENTS) {
      const inserted = await this.ingestLatestCandles(instrument, resolution);
  ...
  async snapshotOptionChain(currency: 'BTC' | 'ETH'): Promise<OptionSnapshotResult> {
    ...
    for (let i = 0; i < activeInstruments.length; i += BATCH) {
      ...
      for (let j = 0; j < tickers.length; j++) {
        ...
        if (res.status !== 'fulfilled' || !res.value) { errors++; continue; }
  ...
  private async fetchChartData(
    ...
    const url = new URL(`${DERIBIT_PUBLIC_REST}/get_tradingview_chart_data`);
    ...
    try {
      ...
      this.logger.warn(`fetchChartData failed for ${instrument}@${resolution}: ${err.message}`);
      return null;
    }
  }
