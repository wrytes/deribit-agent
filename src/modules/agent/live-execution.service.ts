...
export class LiveExecutionService {
	...
	async snapInstrument(
		...
		try {
			...
			const nearest = filtered.reduce((best, inst) =>
				Math.abs(inst.strike - targetStrike) <
			...
			this.logger.error(
				`snapInstrument failed: ${err?.message ?? String(err)}`,
			);
			return null;
		}
	}
	...
	async placeAndAwaitFill(
		...
		try {
			...
			const result = this.deribitClient.unwrap(res) as {
				order: any;
				trades: any[];
			};
			...
			if (!trades.length) {
				this.logger.warn(
					`${side} ${amount} ${instrumentName} — no fills (IOC rejected)`,
				);
				return empty;
			}
			...
			const fillPrice =
				trades.reduce(
					(s: number, t: any) => s + (t.price ?? 0) * (t.amount ?? 0),
					0,
				) / totalAmount;
			const fee = trades.reduce(
				(s: number, t: any) => s + (t.fee ?? 0),
				0,
			);
			...
			this.logger.error(
				`placeAndAwaitFill failed (${side} ${instrumentName}): ${err?.message ?? String(err)}`,
			);
			return empty;
		}
	}
	...
	async markOpenPositions(
		...
		const ts = (offset: number) =>
			new Date(currentDate.getTime() + offset)
				.toISOString()
				.replace(/(\.\d{3})Z$/, '$1Z');
		...
		for (const pos of openPositions) {
			...
			if (dte === 0) {
				...
				const openFeePerUnit =
					pos.openFeeBtc / Math.max(pos.size, 1e-8);
				const pnl =
					(pos.entryPremBtc - intrinsicBtc - openFeePerUnit) *
					pos.size;
				marginBalance += -intrinsicBtc * pos.size;
				settledPnlBtc += pnl;

				const liability = updatedPositions.reduce(
					(s, p) => s + p.size * p.lastMktPrem,
					0,
				);
				entries.push({
			...
			try {
				const client = await this.deribitClient.getClient(userId);
				const posRes = await client.account.getPosition({
					instrument_name: pos.label,
				});
				const posData = this.deribitClient.unwrap(posRes) as any;
				if (posData?.mark_price != null)
					markPriceBtc = Number(posData.mark_price);
