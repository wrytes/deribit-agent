...
export class AgentService {
	...
	async createRun(dto: CreateRunDto) {
		...
		if (runType === AgentRunType.LIVE && !dto.deribitAccountId) {
			throw new BadRequestException(
				'deribitAccountId is required for LIVE agents',
			);
		}
		if (runType !== AgentRunType.LIVE && !dto.initialCapitalBtc) {
			throw new BadRequestException(
				'initialCapitalBtc is required for BACKTEST and PAPER agents',
			);
		}
		...
		if (runType === AgentRunType.LIVE && dto.deribitAccountId) {
			const account = await this.prisma.deribitAccount.findUnique({
				where: { id: dto.deribitAccountId },
			});
			if (account) {
				try {
					const client = await this.deribitClient.getClient(
						account.userId,
					);
					...
					const fetched = Number(
						summary.equity ?? summary.balance ?? 0,
					);
					if (fetched > 0) {
	...
	async stopRun(userId: string, id: string) {
		...
		if (
			...
			throw new BadRequestException(
				`Cannot stop an agent with status ${run.status}`,
			);
		}
	...
	async logActionBatch(
		...
		opts?: {
			...
			paperState?: Record<string, unknown>;
		},
	) {
		...
		if (!run) throw new NotFoundException('Agent not found');
		if (!actions.length && !opts?.currentCapitalBtc && !opts?.paperState)
			return { logged: 0 };
		...
		const totalPnl = actions.reduce(
			(s, a) => s + (Number(a.pnlBtc) || 0),
			0,
		);
		await this.prisma.agentRun.update({
			...
			data: {
				...
				...(totalPnl
					...
					: {}),
				...(opts?.currentCapitalBtc !== undefined
					? { currentCapitalBtc: opts.currentCapitalBtc }
					: {}),
				...(opts?.paperState !== undefined
					? { paperState: opts.paperState as any }
					: {}),
			},
		});
	...
	async executeRun(
		...
		if (!canRerun && run.status !== AgentRunStatus.ACTIVE) {
			throw new BadRequestException(
				`Agent is not active (status: ${run.status})`,
			);
		}
		if (!run.sessionId) {
			throw new BadRequestException(
				'Agent has no linked training session — set sessionId when creating the agent',
			);
		}
		...
		const sessionEnv =
			((run.session?.hyperparams as any)?.env as Record<string, any>) ??
			{};
		const {
			dataFrom: storedFrom,
			dataTo: storedTo,
		...
		const envOverrides =
			Object.keys(envPart).length > 0 ? envPart : undefined;

		const job = await this.agentRunQueue.add(
			'execute',
			{
				runId: id,
				dataFrom: effectiveDataFrom,
				dataTo: effectiveDataTo,
				envOverrides,
			},
			{
				attempts: 2,
