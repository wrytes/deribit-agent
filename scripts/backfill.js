...
const jobs =
	instrument && resolution ? [{ instrument, resolution }] : DEFAULT_JOBS;
...
async function backfill(client, instrument, resolution, fromTs, toTs) {
	...
	if (SPOT_INSTRUMENTS.has(instrument)) {
		if (resolution !== '1D') {
			console.log(
				`  Skipped: ${instrument} only available at 1D (Deribit delivery prices are daily)`,
			);
			return 0;
		}
...
(async () => {
	...
	console.log(
		`Backfill  ${fromDate.toISOString().slice(0, 10)} → ${toDate.toISOString().slice(0, 10)}`,
	);
	console.log(
		`Jobs: ${jobs.map((j) => `${j.instrument}@${j.resolution}`).join(', ')}\n`,
	);
